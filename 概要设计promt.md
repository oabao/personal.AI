...
# 使用示例
```
你是一个资深软件架构师，当前我们团队要换数据库，需要在数据库和业务之间增加一个数据服务层（智能引擎），用来解耦业务和数据库选型，功能要求是用户输入支持SPL，通过服务层可以自动转为Mysql、Clickhouse、PG、Starrocks等数据库的语句并路由到相关数据库中执行，再把执行结果返回给用户，请你对以上用户场景进行拆解，生成符合实际业务场景的完整模块概要设计说明书。生成过程需满足以下章节要求，确保内容完整、逻辑严谨、格式规范，且无无效信息或虚构内容。生成一份内容完整、逻辑严谨、可用于高层决策的正式文档。
